...
}; // 추가 프로퍼티를 넣어야 하는 경우 타입 단언으로 해결 가능 (타입 추론 가능)
/*
    2. 💡 타입 단언 규칙
        - 값 as 타입 (단언식) : A as B
...
*/
let num1 = 10; // number(슈퍼타입) - never(서브타입)
let num2 = 10; // number(서브타입) - unknown(슈퍼타입)
// let num3 = 10 as string; // 오류 발생 : number - string 은 서로 교집합이 없으므로 서로 슈퍼-서브 타입이 아님
// 해결 방법 (좋은 방법은 아님) - 다중 단언
let num3 = 10; // number(서브타입) - unknown(슈퍼타입) -> unknown(슈퍼타입) - string(서브타입) : 다중 단언
/*
    3. const 단언
...
*/
let num4 = 10; // number literal 타입 10으로 추론
let cat = {
    name: "야옹이",
    color: "yellow",
}; // readonly 프로퍼티
...
const len = post.author.length; // ! : Non null 단언 연산자 (해당 값이 null이거나 undefined이 아닐 것이라고 타입 컴파일러에게 단언 (string 타입으로 단언))
export {};
