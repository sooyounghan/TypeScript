...
/*
    2. 💡 타입 단언 규칙
        ...
        - 서로소 집합의 경우 겹치는 부분이 없으므로 오류 발생 가능성이 높으므로 불가가
*/
...
/*
    3. const 단언 
        ...
        - 객체 타입과 함께 사용하면 활용 가능
            + 객체 변수 선언 후 as const를 하면 해당 프로퍼티들이 readonly, 읽기 전용 프로퍼티로 변하여 읽기 전용 객체로 추론론
*/
...
const len: number = post.author!.length; // ! : Non null 단언 연산자 (해당 값이 null이거나 undefined이 아닐 것이라고 타입 컴파일러에게 단언 (string 타입으로 단언언))
```
